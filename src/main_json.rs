...
impl Todos{
    ...
    pub fn text_format(&mut self) -> String{
        let mut formated: String = "".to_string();
        for i in 0..self.todos.len(){
            formated+=format!("{}<{}> - {}\n", i+1, if self.marks[i] {"*"} else {" "}, self.todos[i]).as_str();
        }
        formated
    }

    fn load(&mut self, filename: &str, mode: i32) -> Result<()>{
